...
export const register = async (req, res) => {
    // const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;  --> if email-validator package does not work
    ...
    if (data.password != data.confirmPassword) {
        return res.status(400).json({ status: 'error', message: 'Password and confirm password doesn\'t match' });
    }
...
export const login = async (req, res) => {
    ...
    if (await bcrypt.compare(password, user.password)) {
        ...
        return res.status(200).json({ status: 'ok', message: 'Login confirmed', user: { id: user._id, name: user.name, lastname: user.lastname } });
    }
